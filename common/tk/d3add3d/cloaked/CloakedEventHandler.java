...
public class CloakedEventHandler implements IWorldGenerator {
	...
	private void generateEnd(World world, Random random, int x, int z) {
		if(MiscConfigs.MISC_generateEnd==1) {
			this.addOreSpawnEnd(CloakedCore.oneBlock, world, random, x, z, 16, 16, 9 + random.nextInt(3), 15, 5, 128);
		}
	}

	private void generateSurface(World world, Random random, int x, int z) {
		this.addOreSpawn(CloakedCore.oneBlock, world, random, x, z, 16, 16, 9 + random.nextInt(3), 5, 5, 13);
	}

	private void generateNether(World world, Random random, int x, int z) {
		if(MiscConfigs.MISC_generateNether==1) {
			this.addOreSpawnNether(CloakedCore.oneBlock, world, random, x, z, 16, 16, 9 + random.nextInt(3), 5, 5, 128);
		}
	}
	       ...
	       public void addOreSpawnEnd(Block block, World world, Random random, int blockXPos, int blockZPos, int maxX, int maxZ, int maxVeinSize, int chancesToSpawn, int minY, int maxY)
	       {
	             @SuppressWarnings("unused")
				int maxPossY = minY + (maxY - 1);
	             assert maxY > minY: "The maximum Y must be greater than the Minimum Y";
